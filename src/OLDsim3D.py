...
# Constants
g = 9.81 # on moon or on mars
dt = 0.02 # TODO: try 10x smaller
k_d = 0.6   # drag coefficient - find citation for 0.5 to 0.65 = c_d of cricket ball
...
diameter_of_ball = circumference_of_ball / math.pi  # meters
...
speed = 20  # m/s
angle_with_pos_x = math.radians(90)   # horizontal angle (90° points along +z axis)
angle_with_pos_z = math.radians(10)   # elevation angle
...
# Constant angular velocity throughout - assumption since "no evidence to support how this slows"
angular_velocity = 170  # rad/s for off spinnner? (27 rev/s * 2 pi = 170 rad/s)
# negative omega[0] indicates backspin, positive would be topspin
omega = [angular_velocity, 0]  # assume constant - around x, around y
# TODO : angular velocity is magnitude of omega - it should have components - put in 'omega' variable
...
def flight(omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z):
    ...
    angle_with_pos_x = math.atan2(vy, vx) 
    angle_with_pos_z = math.atan2(vy, vz)

    return omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z

def bounce(omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z, h):
    ...
    bounce_dt = 2*e_number*math.sqrt(2*h/g)
    # This is 3D
    vy_new = k**0.5 * vy *-1
    vx_new = np.sign(omega[0])*(friction_coefficient * (vy + vy_new) + friction_coefficient*g*bounce_dt) + (vx + radius_of_ball*omega[0])
    vz_new = np.sign(omega[0])*(friction_coefficient * (vy + vy_new) + friction_coefficient*g*bounce_dt) + (vz + radius_of_ball*omega[0])

    angle_with_pos_x = math.atan2(vz, vx) 
    angle_with_pos_z = math.atan2(vy, vz)

    omega = np.array(omega)*(2/7)  #assuming ball is solid sphere
...
def display():  # I think the display is reflected in x axis (down line of bowler's view is +x)
    ...
    global past_x_values, past_y_values, past_z_values, maximum_height
    global radius_of_ball
    global stop_simulation
    scale = 8  # for simulation visuals
    scaled_radius_of_ball = scale * radius_of_ball
    ...
    if not stop_simulation:
        ...
        if z > batter_stumps_distance:
            ...
            omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z, bounce_dt = bounce(omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z, maximum_height)
        ...
        if x > maximum_height:
            maximum_height = x  # finds when elevation angle is zero (otherwise it finds the initial x value)
...
def next_sim(case, viewpoint):
    case += 1

    global directory_test_case_results, images
    stop_simulation = False
    # Iterate over all test cases
    with open("../test/test_cases.txt", "r") as test_cases_file:
        lines = test_cases_file.readlines()[case:]  # Skip header line OR CHANGE TO ACCESS OTHER TEST CASES FOR NOW
        for line in lines:
            params = line.strip().split(';')
            dt = float(params[0])
            k_d = float(params[1])
            initial_speed = float(params[2])
            elevation_angle = float(params[3])
            horiztonal_angle = float(params[4])
            initial_omega = [float(params[5]), float(params[6])]
            description = params[7]
            print("Testing case:", description)

            # Set initial conditions based on test case
            speed = initial_speed
            angle_with_pos_x = horiztonal_angle
            ...
            angular_velocity = initial_omega[0]  # assuming only x component for now
            omega = [angular_velocity, initial_omega[1]]
            ...
            print(f"Running simulation for test case: {description}")
...
if __name__ == "__main__":
    ...
    global viewpoint
    viewpoint = "bowler"  # other options: "bowler", "diagonal", "top-down"
    ...
    with open("../test/test_cases.txt", "r") as test_cases_file:
        ...
        for line in lines:
            ...
            initial_omega = [spin_rate*math.cos(horizontal_angle), spin_rate*math.sin(horizontal_angle)]

...
k_d = 0.5   # drag coefficient - find citation for 0.5 = c_d of cricket ball
...
bowler_release_distance = -pitch_length + pitchstart_to_poppingcrease - half_bowler_stride
...
speed = 22.4  # m/s (about 50 mph)
angle_with_pos_x = math.radians(90)   # horizontal angle (90° points along +z axis)
angle_with_pos_z = math.radians(30)   # elevation angle
...
def bounce(omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z):
    ...
    vx_new = vx
    vy_new = k**0.5 * vy *-1
    vz_new = np.sign(omega[0])*(friction_coefficient * (vy + vy_new) + friction_coefficient*g*dt) + (vz + radius_of_ball*omega[0])

    angle_with_pos_x = angle_with_pos_x #math.atan2(vy, vx) 
    angle_with_pos_z = math.atan2(vy, vz)

    return omega, vx_new, vy_new, vz_new, angle_with_pos_x, angle_with_pos_z
...
def display():  # I think the display is reflected in x axis (down line of bowler's view is +x)
    ...
    if not stop_simulation:
        ...
        if x > pitch_length: #batter_stumps_distance:
            ...
            omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z = bounce(omega, vx, vy, vz, angle_with_pos_x, angle_with_pos_z)
        ...
        glBegin(GL_QUADS)
        glNormal3f(0,1,0) # normal vector pointing up - indicates orientation of ground
        glVertex3f(-pitch_width, 0, -pitch_length)
        glVertex3f( pitch_width, 0, -pitch_length)
        glVertex3f( pitch_width, 0,  pitch_length)
        glVertex3f(-pitch_width, 0,  pitch_length)
        glEnd()
...
def plot_main():
    ...
    ax.set_ylim(-pitch_length, pitch_length*2) # TODO: pitch length is double what it should be - see calculations
    ax.set_xlabel("x (m)")
    ax.set_ylabel("z (m)")
    ax.set_zlabel("y (m)")
